...
# The Node host imports only sdk.mjs. Browser/bridge clients, Bun extraction
# helpers and TypeScript declarations are not used by Workass's runtime.
# Keep the package metadata and both licensing documents intact.
claude_destination="$incoming/node_modules/@anthropic-ai/claude-agent-sdk"
mkdir -p "$claude_destination"
for sdk_file in sdk.mjs package.json LICENSE.md README.md; do
  cp "$sdk_source/$sdk_file" "$claude_destination/$sdk_file"
done

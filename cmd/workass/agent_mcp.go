...
func agentMCPTools() []map[string]any {
	...
	return []map[string]any{
		...
		tool("workass_create_chat", "Create a durable Workass chat. Selection and cwd inherit the calling chat unless explicitly supplied and validated against the live catalog. Does not focus the UI unless focus=true.", object(map[string]any{
		...
		tool("workass_rename_chat", "Rename one exact Workass chat without changing the active tab or any other chat state.", object(map[string]any{
		...
		tool("workass_configure_chat", "Set cwd/provider/model/effort/permission for one exact chat. Every selection is validated against the live provider catalog and the resolved native ids are returned.", object(map[string]any{
			"tab_id":            str("Tab id from workass_list_chats."),
		...
		tool("workass_focus_chat", "Focus one exact Workass chat in the user's UI. This is the only chat tool that changes the active tab by default.", object(map[string]any{
			"tab_id": str("Tab id from workass_list_chats."), "chat_id": str("Paired conversation id."),
		}, "tab_id", "chat_id"), false, false, true, false),
		tool("workass_delete_chat", "Delete one exact Workass chat and its native binding. Refuses a running turn unless force=true explicitly authorizes cancellation and deletion.", object(map[string]any{
			"tab_id": str("Tab id from workass_list_chats."), "chat_id": str("Paired conversation id."),
			"force": boolean("Cancel a running turn before deleting."),
		}, "tab_id", "chat_id"), false, true, false, false),
		tool("workass_send_chat_message", "Send text to one exact chat without focusing it. auto sends at the next idle boundary, queue always preserves FIFO order, and steer explicitly targets the active turn with provider-native semantics plus durable fallback.", object(map[string]any{
			"tab_id":   str("Tab id from workass_list_chats."),
			"chat_id":  str("Paired conversation id."),
			"message":  str("Message to send."),
			"delivery": enum("Delivery behavior; defaults to auto.", "auto", "queue", "steer"),
		}, "tab_id", "chat_id", "message"), false, false, false, true),
		tool("workass_cancel_chat_turn", "Cancel the currently running turn in one exact chat. Does not delete queued follow-ups.", object(map[string]any{
			"tab_id": str("Tab id from workass_list_chats."), "chat_id": str("Paired conversation id."),
		}, "tab_id", "chat_id"), false, true, false, false),
		...
		tool("workass_host_artifact", "Host a file or static directory from the calling agent's Workass cwd. Returns a stable URL and ready-to-use markdown; put that markdown in your response rather than a local path. Any file type. A directory defaults to index.html, else pass entry.", object(map[string]any{
		...
		tool("workass_wait_subagent", "Wait for one tracked subagent. A child permission request forcibly ends the wait with needsAttention=true, which you must read before continuing; otherwise returns the final result or error.", object(map[string]any{
		...
		}, "subagent_id"), true, false, false, false),
		tool("workass_wait_subagents", "Wait for the first or all selected subagents; returns completed plus still-running snapshots. A child permission request ends the wait with a latched attention list to read. Timeout is a normal result, not an error.", object(map[string]any{
			...
			"timeout_ms":   map[string]any{"type": "integer", "minimum": 1000, "maximum": 3600000, "description": "Wait timeout in milliseconds; defaults to 10 minutes."},
		}, "subagent_ids"), true, false, false, false),
		tool("workass_message_subagent", "Send coordinator feedback to a running subagent. Uses acknowledged live steering when supported; otherwise persists an immediate follow-up before interrupting/queuing.", object(map[string]any{
		...
		tool("workass_retry_subagent", "Retry a completed/failed/cancelled subagent with the same resolved selection and optional additional guidance.", object(map[string]any{
		...
		tool("workass_settle_external_work", "Mark a registered external lane finished. Repeating the same settle is idempotent.", object(map[string]any{
		...
		tool("workass_cancel_subagent", "Cancel one tracked subagent owned by the current Workass turn.", object(map[string]any{
			"subagent_id": str("Subagent id returned by spawn/list."),
		}, "subagent_id"), false, true, false, false),
		tool("workass_decide_subagent_permission", "Answer a permission request from one of your tracked subagents rather than leaving it parked on a human. You may always deny; you may allow only what this chat's own mode already does without asking.", object(map[string]any{

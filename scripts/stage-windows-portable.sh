...
for shell_file in main.js preload.js view-server.js connected-artifacts.js browser-manager.js browser-viewport.js browser-observation.js browser-control-server.js runtime-profile.js runtime-bootstrap.js certificate-pins.js app-icon.js image-copy.js profile-singleton.js update-lock-recovery.js update-progress.js update-manager.js update-worker.js; do
  cp "$repo_root/desktop/shell/$shell_file" "$stage/resources/app/$shell_file"
done
node "$repo_root/desktop/scripts/check-shell-dependencies.cjs" "$stage/resources/app"

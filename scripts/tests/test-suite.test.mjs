...
test('full suite partitions every renderer test across six sequential isolated Node processes', () => {
  ...
  assert.deepEqual(scheduled, [...measuredHeavy, ...inventory.filter(name => !measuredHeavy.includes(name))]);
  assert.equal(new Set(scheduled).size, inventory.length, 'every script test is scheduled exactly once');
  assert.deepEqual(scheduled.slice().sort(), inventory, 'scheduled script tests match the complete inventory');
  ...
  assert.equal(scripts.args.filter(arg => arg.endsWith('.test.mjs')).length, inventory.length);

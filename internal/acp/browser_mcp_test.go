...
func TestBrowserMCPServerIsInjectedForEveryConfiguredACPSession(t *testing.T) {
	...
	}, SessionOptions{ChatID: "chat-any-provider", TabID: "tab-any-provider", AgentOwnerKey: "owner-any-provider"})
	if len(servers) != 1 {
...
func TestBrowserMCPInjectionRejectsPlaintextURL(t *testing.T) {
	if got := browserMCPServers(Options{WorkassMCPBaseURL: "http://127.0.0.1:8788"}, SessionOptions{AgentOwnerKey: "owner"}); len(got) != 0 {
...
func TestSubagentBriefOmitsTheBrowserItDoesNotHave(t *testing.T) {
	...
	if servers := browserMCPServers(Options{WorkassMCPBaseURL: "https://localhost:8788"},
		SessionOptions{ChatID: subagentChatIDPrefix + "abc", AgentOwnerKey: "owner"}); len(servers) != 0 {

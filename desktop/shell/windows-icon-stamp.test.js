...
test('the tracked Workass ICO has taskbar sizes and a compact large fallback', async () => {
  ...
  assert.ok(images.find((image) => image.width === 128).data.length < 18_963);
});
...
test('the pinned Windows Electron executable is stamped and independently re-verified', {
  ...
  assert.match(stamped.stdout, /WORKASS_WINDOWS_ICON_STAMPED sizes=16,32,48,128/);
  const verified = spawnSync(process.execPath, [script, '--verify', '--exe', executable, '--icon', icon], { encoding: 'utf8' });
  assert.equal(verified.status, 0, verified.stderr);
  assert.match(verified.stdout, /WORKASS_WINDOWS_ICON_VERIFIED sizes=16,32,48,128/);

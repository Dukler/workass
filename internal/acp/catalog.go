...
func normalizeClaudeCatalogModels(models []Model) []Model {
	...
	// Claude models. Showing both makes that model appear twice and gives the
	// user two labels for the same selection, so keep only explicit model IDs.
...
// resolveClaudeSyntheticDefaultAlias uses only the normalized, pre-filter
// provider catalog to identify Claude's synthetic default row. The alias is
// authoritative only when exactly one explicit model has the same
// provider-derived display name. Catalog order is never a fallback.
...
// reconcileClaudeLiveCatalog protects the model-id set learned by the complete
// ephemeral provider probe from a partial live-session config surface. A
// resumed older Claude session can report the root alias (for example
// claude-fable-5) where a fresh probe advertises the explicit literal context
// model (claude-fable-5[1m]). In that case the explicit id remains authoritative
// and receives the live session's current effort capability.
//
// Missing previously probed models are retained until the next explicit
...
func reconcileClaudeLiveCatalog(previous, current []Model, known map[string]bool) ([]Model, map[string]bool) {
	...
	for _, model := range previous {
		root, ok := literalContextModelRoot(model.ModelID)
		if ok {
...
// providerCatalogModel turns the Claude adapter's authoritative description
// (for example "Sonnet 5" or "Opus 4.8 with 1M context") into the concise,
